...
@WebServlet("/admin/fetch-jobs")
public class JobmoderationServlet extends HttpServlet {
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            while (rs.next()) {
                ...
                job.put("status", rs.getObject("status", String.class)); // Include job status
                jobList.add(job);
            }
            ...
            for (int i = 0; i < jobList.size(); i++) {
                ...
                out.print("\"status\":\"" + job.get("status") + "\""); // Include job status
                out.print("}");
